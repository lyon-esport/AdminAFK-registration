...
function new_crsf($name)
{
    $ssl_size = rand(10, 30);
    $bytes = openssl_random_pseudo_bytes($ssl_size, $cstrong);
    $value = bin2hex($bytes);
    $_SESSION[$name] = $value;
    return array('name' => $name, 'value' => $value);
}

function check_csrf($name)
{
    if(!isset($_SESSION[$name]) || !isset($_POST[$name]) || empty($_SESSION[$name]) || empty($_POST[$name]) || ($_SESSION[$name] !== $_POST[$name]))
    {
        $_SESSION[$name]=' ';
        unset($_SESSION[$name]);
        return false;
